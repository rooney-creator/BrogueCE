...
#endif
    "--variant variant_name     run a variant game (options: rapid_brogue, bullet_brogue, volatile_brogue)\n"
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--variant") == 0) {
            if (i + 1 < argc) {
                ...
                } else if (!strcmp("bullet_brogue", argv[i + 1])) {
                    gameVariant = VARIANT_BULLET_BROGUE;
                } else if (!strcmp("volatile_brogue", argv[i + 1])) {
